...
window.renderApp = function renderApp() {
    ...
    [0, 1].forEach(i => {
        ...
        ${grid}
        ${m ? `<div class="staff-pill med-pill">Dr. ${escapeHTML(m.firstName)} ${escapeHTML(m.lastName).toUpperCase()} ${medEditable ? `<span class="remove-btn" onclick="clearShift(${i},'med')">\u00D7</span>` : ''}</div>` : `<div class="search-box">${medEditable ? `<input type="text" id="search-med-${i}" placeholder="\uD83D\uDD0D Nom Doc..." class="special-input" oninput="doSearch('med-${i}',this.value)" autocomplete="off"><div class="suggestions" id="sugg-med-${i}"></div>` : '<span style="color:var(--text-muted);font-size:0.8rem;font-weight:700;">---</span>'}</div>`}
      </div>`;
    });
    ...
    boardHTML += `<div class="bed-card special-card med-card ${t.uAllC ? (h.adminLockUsip ? 'admin-closed' : 'closed') : ''}" style="${t.uAllC ? 'pointer-events:none;' : ''}">
      ...
      ${uGrid}
      ${m2 ? `<div class="staff-pill med-pill">Dr. ${escapeHTML(m2.firstName)} ${escapeHTML(m2.lastName).toUpperCase()} ${locked ? '' : `<span class="remove-btn" onclick="clearShift(2,'med')">\u00D7</span>`}</div>` : `<div class="search-box">${locked ? '---' : `<input type="text" id="search-med-2" class="special-input" placeholder="\uD83D\uDD0D Nom Doc..." oninput="doSearch('med-2',this.value)" autocomplete="off"><div class="suggestions" id="sugg-med-2"></div>`}</div>`}
      `}
    </div>`;
